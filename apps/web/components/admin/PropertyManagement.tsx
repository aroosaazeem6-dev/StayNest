...
export function PropertyManagement() {
  ...
  const [state, setState] = useState<{
    ...
    data: AdminProperty[] | null;
    meta: AdminPropertyListResponse['meta'] | null;
    error: string | null;
  ...
  useEffect(() => {
    ...
    adminService
      .listProperties(tokens.accessToken, {
        page,
      ...
      .catch((err: unknown) => {
        ...
        if (!mounted) return;
        setState({ status: 'error', data: null, meta: null, error: e?.message || 'Unable to load properties.' });
      });
    return () => {
      mounted = false;
    };
  }, [page, status, search, tokens]);
          ...
          <tbody className="divide-y divide-gray-100">
            {state.data.map((property) => (
              <tr key={property.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-700">{property.propertyType}</td>
                <td className="px-4 py-3 text-gray-700">
                  {[property.city, property.country].filter(Boolean).join(', ') || '—'}
                </td>
                <td className="px-4 py-3 text-gray-700">${property.pricePerNight}</td>
                <td className="px-4 py-3">
                  <StatusSelect property={property} tokens={tokens} />
                </td>
...
function StatusSelect({ property, tokens }: { property: AdminProperty; tokens: { accessToken?: string } | null }) {
  ...
  async function handleChange(newStatus: string) {
    ...
    setError(null);
    try {
      const updated = await adminService.updatePropertyStatus(accessToken, property.id, newStatus);
